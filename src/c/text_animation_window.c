...
static void out_stopped_handler(Animation *animation, bool finished,
                                void *context) {
  ...
  GRect start = GRect(frame.origin.x + (2 * TEXT_ANIMATION_WINDOW_DISTANCE),
                      frame.origin.y, frame.size.w, frame.size.h);
  GRect finish = GRect(frame.origin.x + TEXT_ANIMATION_WINDOW_DISTANCE,
                       frame.origin.y, frame.size.w, frame.size.h);

  PropertyAnimation *in_prop_anim =
...
static void shake_animation() {
  ...
  GRect finish = GRect(start.origin.x - TEXT_ANIMATION_WINDOW_DISTANCE,
                       start.origin.y, start.size.w, start.size.h);

  PropertyAnimation *out_prop_anim =
      property_animation_create_layer_frame(text_layer, &start, &finish);
  ...
  animation_set_handlers(
      out_anim, (AnimationHandlers){.stopped = out_stopped_handler}, NULL);
  animation_schedule(out_anim);
}

static void destroy_callback(void *context) {
...
static void animate_callback(void *context) {
  ...
  if (s_current_text == 1) {
    s_timer = app_timer_register(2000, destroy_callback, NULL);
  } else {
    animate();
  }
}
...
static void animate() {
  shake_animation();
  s_timer = app_timer_register(TEXT_ANIMATION_WINDOW_INTERVAL, animate_callback,
                               NULL);
}
...
static void window_load(Window *window) {
  ...
  text_layer_set_text(s_text_layer, "You finished a mountain!");
...
void text_animation_window_push(char *line1, char *line2) {
